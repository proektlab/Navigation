...
def loss(predicted, targets, classes, plotPanel=None):    
    ...
    finalLoss = torch.nn.MSELoss()       
    
    taskScore = finalLoss(predicted, targets)

    accuracy = np.corrcoef(predicted.view(-1).detach().numpy(), targets.view(-1).detach().numpy(), rowvar=True)
    accuracy = torch.ones(1) * accuracy[0,1]
    
    if not plotPanel is None:
        plotPanel.clear()
        plotPanel.plot(predicted.detach().numpy()[0,:,:], 'g-')   
        plotPanel.plot(targets.detach().numpy()[0,:,:], 'b-',)
...
if __name__ == "__main__":
    ...
    #        experimentParameters = '{}parameters.pkl'.format(experimentDir)
            ...
            model = NNNModel(inputSize, targetData.shape[0], NUM_NEURONS, MODULE_COUNT, MODULE_CONNECTEDNESS)
